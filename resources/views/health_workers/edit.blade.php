...
<div class="max-w-md mx-auto py-10">
    ...
    <form method="POST" action="{{ route('health_workers.update', $health_worker->id) }}" class="space-y-4 bg-white shadow p-6 rounded-lg">
        @csrf @method('PUT')
        <div>
            <label>Nama</label>
        ...
        <div>
            <label>Wilayah</label>
            <input type="text" name="region" class="w-full border rounded p-2" value="{{ old('region', $health_worker->region) }}" required>
