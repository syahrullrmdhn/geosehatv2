...
<div class="max-w-3xl mx-auto py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('health_workers.create') }}"
        ...
        <table class="min-w-full text-sm">
            <thead>
                <tr class="bg-gray-50 text-gray-600 uppercase">
                    ...
                    <th class="px-4 py-2 text-left">Deskripsi Wilayah</th>
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2">{{ $w->region->name }}</td>
                    <td class="px-4 py-2 text-gray-600">{{ $w->region->description ?? '-' }}</td>
                    <td class="px-4 py-2">{{ $w->phone }}</td>
                    <td class="px-4 py-2 text-center">
                        <a href="{{ route('health_workers.edit', $w) }}" class="text-indigo-600 hover:underline">Edit</a>
                        <form method="POST"
                              action="{{ route('health_workers.destroy', $w) }}"
                              class="inline"
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-4 py-3 text-center text-gray-400">Belum ada data</td>
                </tr>
                @endforelse
            </tbody>
        </table>
